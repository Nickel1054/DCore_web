...
for (const row of rows) {
    let timedata = row.getElementsByClassName('timedata')[0];
    let spreads = row.getElementsByClassName('spreads')[0];

    // create_chart(spreads);
}

function create_chart(element) {

    let root = am5.Root.new(element.id);
